...
export async function middleware(request: NextRequest) {
    ...
    if (entity_type === "404") {
        ...
        let updated_url = request.url.replace(/(source|sample|dataset|upload|collection|publication)/, entity_type)
...
export const config = {
    matcher: [
        '/((?:source|sample|dataset|upload|collection|publication).*)',
        '/edit/((?:source|sample|dataset|upload|collection|publication).*)'
    ],
