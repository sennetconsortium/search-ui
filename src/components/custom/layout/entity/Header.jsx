...
function EntityHeader({entity, data, isEditMode, values, showGroup = true, adminGroup}) {
  ...
  return (
    <Container className="px-0" fluid={true}>
        <Row md={12}>
            <h4>{_t(`${entity} Information`)} {values && values.status && <span className={`${getStatusColor(values.status)} badge`}><SenNetPopover placement={'bottom'} text={getStatusDefinition(values.status)} className={'status-info'}>{values.status}</SenNetPopover></span>}</h4>
        </Row>
